const OthersFolder = require('../models/OthersFolder');
const OthersFile = require('../models/OthersFile');
...
const createFolder = async (req, res) => {
  try {
    ...
    const folder = new OthersFolder({ title, description, createdBy: req.user.id });
    await folder.save();
    await folder.populate('createdBy', 'fullName email');
    res.status(201).json({ success: true, message: 'Others folder created successfully', data: folder });
  } catch (error) {
    console.error('Error creating others folder:', error);
...
const getAllFolders = async (req, res) => {
  try {
    const folders = await OthersFolder.find().populate('createdBy', 'fullName email').sort({ createdAt: 1 });
    res.json({ success: true, data: folders });
  } catch (error) {
    console.error('Error fetching others folders:', error);
...
const getFolderById = async (req, res) => {
  try {
    const folder = await OthersFolder.findById(req.params.id).populate('createdBy', 'fullName email');
    if (!folder) return res.status(404).json({ success: false, message: 'Folder not found' });
    res.json({ success: true, data: folder });
  } catch (error) {
    console.error('Error fetching others folder:', error);
...
const updateFolder = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) return res.status(400).json({ success: false, message: 'Validation errors', errors: errors.array() });
    const { title, description } = req.body;
    const folder = await OthersFolder.findByIdAndUpdate(req.params.id, { title, description }, { new: true, runValidators: true }).populate('createdBy', 'fullName email');
    if (!folder) return res.status(404).json({ success: false, message: 'Folder not found' });
    res.json({ success: true, message: 'Folder updated successfully', data: folder });
  } catch (error) {
    console.error('Error updating others folder:', error);
...
const deleteFolder = async (req, res) => {
  try {
    const folder = await OthersFolder.findById(req.params.id);
    if (!folder) return res.status(404).json({ success: false, message: 'Folder not found' });
    const files = await OthersFile.find({ folder: req.params.id });
    for (const file of files) {
      for (const attachment of file.attachments) {
        try { await cloudinary.uploader.destroy(attachment.publicId); } catch (e) { console.error('Cloudinary error:', e); }
      }
    }
    await OthersFile.deleteMany({ folder: req.params.id });
    await OthersFolder.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'Folder and all its contents deleted successfully' });
  } catch (error) {
    console.error('Error deleting others folder:', error);
...
const createFile = async (req, res) => {
  try {
    ...
    const folderExists = await OthersFolder.findById(folder);
    ...
    const file = new OthersFile({ title, description, content, attachments: attachments || [], sourceLinks: sourceLinks || [], folder, createdBy: req.user.id });
    await file.save();
    ...
    console.error('Error creating others file:', error);
...
const getFolderFiles = async (req, res) => {
  try {
    const files = await OthersFile.find({ folder: req.params.folderId }).populate('createdBy', 'fullName email').populate('folder', 'title').sort({ createdAt: 1 });
    res.json({ success: true, data: files });
  } catch (error) {
    console.error('Error fetching others files:', error);
...
const getFileById = async (req, res) => {
  try {
    const file = await OthersFile.findById(req.params.id).populate('createdBy', 'fullName email').populate('folder', 'title');
    if (!file) return res.status(404).json({ success: false, message: 'File not found' });
    res.json({ success: true, data: file });
  } catch (error) {
    console.error('Error fetching others file:', error);
...
const updateFile = async (req, res) => {
  try {
    ...
    const file = await OthersFile.findByIdAndUpdate(req.params.id, { title, description, content, attachments: attachments || [], sourceLinks: sourceLinks || [] }, { new: true, runValidators: true }).populate([{ path: 'createdBy', select: 'fullName email' }, { path: 'folder', select: 'title' }]);
    if (!file) return res.status(404).json({ success: false, message: 'File not found' });
    res.json({ success: true, message: 'File updated successfully', data: file });
  } catch (error) {
    console.error('Error updating others file:', error);
...
const deleteFile = async (req, res) => {
  try {
    const file = await OthersFile.findById(req.params.id);
    if (!file) return res.status(404).json({ success: false, message: 'File not found' });
    for (const attachment of file.attachments) {
      try { await cloudinary.uploader.destroy(attachment.publicId); } catch (e) { console.error('Cloudinary error:', e); }
    }
    await OthersFile.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'File deleted successfully' });
  } catch (error) {
    console.error('Error deleting others file:', error);
