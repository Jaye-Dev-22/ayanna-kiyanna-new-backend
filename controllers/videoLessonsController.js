const VideoLessonsFolder = require('../models/VideoLessonsFolder');
const VideoLessonsFile = require('../models/VideoLessonsFile');
...
const createFolder = async (req, res) => {
  try {
    ...
    const folder = new VideoLessonsFolder({ title, description, createdBy: req.user.id });
    await folder.save();
    await folder.populate('createdBy', 'fullName email');
    res.status(201).json({ success: true, message: 'Video lessons folder created successfully', data: folder });
  } catch (error) {
    console.error('Error creating video lessons folder:', error);
...
const getAllFolders = async (req, res) => {
  try {
    const folders = await VideoLessonsFolder.find().populate('createdBy', 'fullName email').sort({ createdAt: 1 });
    res.json({ success: true, data: folders });
  } catch (error) {
    console.error('Error fetching video lessons folders:', error);
...
const getFolderById = async (req, res) => {
  try {
    const folder = await VideoLessonsFolder.findById(req.params.id).populate('createdBy', 'fullName email');
    if (!folder) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    res.json({ success: true, data: folder });
  } catch (error) {
    console.error('Error fetching video lessons folder:', error);
...
const updateFolder = async (req, res) => {
  try {
    ...
    const { title, description } = req.body;
    const folder = await VideoLessonsFolder.findByIdAndUpdate(req.params.id, { title, description }, { new: true, runValidators: true }).populate('createdBy', 'fullName email');
    if (!folder) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    res.json({ success: true, message: 'Folder updated successfully', data: folder });
  } catch (error) {
    console.error('Error updating video lessons folder:', error);
...
const deleteFolder = async (req, res) => {
  try {
    const folder = await VideoLessonsFolder.findById(req.params.id);
    if (!folder) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    const files = await VideoLessonsFile.find({ folder: req.params.id });
    ...
    await VideoLessonsFile.deleteMany({ folder: req.params.id });
    await VideoLessonsFolder.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'Folder and all its contents deleted successfully' });
  } catch (error) {
    console.error('Error deleting video lessons folder:', error);
...
const createFile = async (req, res) => {
  try {
    ...
    const folderExists = await VideoLessonsFolder.findById(folder);
    if (!folderExists) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    ...
    console.error('Error creating video lessons file:', error);
...
const getFolderFiles = async (req, res) => {
  try {
    const files = await VideoLessonsFile.find({ folder: req.params.folderId }).populate('createdBy', 'fullName email').populate('folder', 'title').sort({ createdAt: 1 });
    res.json({ success: true, data: files });
  } catch (error) {
    console.error('Error fetching video lessons files:', error);
...
const getFileById = async (req, res) => {
  try {
    const file = await VideoLessonsFile.findById(req.params.id).populate('createdBy', 'fullName email').populate('folder', 'title');
    if (!file) {
      return res.status(404).json({ success: false, message: 'File not found' });
    }
    res.json({ success: true, data: file });
  } catch (error) {
    console.error('Error fetching video lessons file:', error);
...
const updateFile = async (req, res) => {
  try {
    ...
    console.error('Error updating video lessons file:', error);
...
const deleteFile = async (req, res) => {
  try {
    const file = await VideoLessonsFile.findById(req.params.id);
    if (!file) {
      return res.status(404).json({ success: false, message: 'File not found' });
    }
    for (const attachment of file.attachments) {
    ...
    await VideoLessonsFile.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'File deleted successfully' });
  } catch (error) {
    console.error('Error deleting video lessons file:', error);
