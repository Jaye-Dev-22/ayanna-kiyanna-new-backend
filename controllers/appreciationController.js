...
const AppreciationFolder = require('../models/AppreciationFolder');
const AppreciationFile = require('../models/AppreciationFile');
const AppreciationComment = require('../models/AppreciationComment');
...
const createFolder = async (req, res) => {
  try {
    ...
    const folder = new AppreciationFolder({ title, description, createdBy: req.user.id });
    await folder.save();
    ...
    console.error('Error creating appreciation folder:', error);
...
const getAllFolders = async (req, res) => {
  try {
    const folders = await AppreciationFolder.find({ isActive: true })
      .populate('createdBy', 'fullName email')
      .sort({ createdAt: 1 });
    res.json({ success: true, data: folders });
  } catch (error) {
    console.error('Error fetching appreciation folders:', error);
...
const getFolderById = async (req, res) => {
  try {
    const folder = await AppreciationFolder.findById(req.params.id)
      .populate('createdBy', 'fullName email');
    if (!folder) {
    ...
    console.error('Error fetching appreciation folder:', error);
...
const updateFolder = async (req, res) => {
  try {
    ...
    const { title, description } = req.body;
    const folder = await AppreciationFolder.findByIdAndUpdate(
      req.params.id,
      { title, description },
      { new: true }
    ).populate('createdBy', 'fullName email');
    ...
    console.error('Error updating appreciation folder:', error);
...
const deleteFolder = async (req, res) => {
  try {
    ...
    console.error('Error deleting appreciation folder:', error);
...
const createFile = async (req, res) => {
  try {
    ...
    const folder = await AppreciationFolder.findById(folderId);
    if (!folder) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    ...
    const file = new AppreciationFile({
    ...
    console.error('Error creating appreciation file:', error);
...
const getFolderFiles = async (req, res) => {
  try {
    const files = await AppreciationFile.find({ folderId: req.params.folderId, isActive: true })
      .populate('createdBy', 'fullName email')
      .populate('folderId', 'title')
      .sort({ createdAt: 1 });
    res.json({ success: true, data: files });
  } catch (error) {
    console.error('Error fetching appreciation files:', error);
...
const getFileById = async (req, res) => {
  try {
    const file = await AppreciationFile.findById(req.params.id)
      .populate('createdBy', 'fullName email')
      .populate('folderId', 'title')
    ...
    console.error('Error fetching appreciation file:', error);
...
const updateFile = async (req, res) => {
  try {
    ...
    const file = await AppreciationFile.findByIdAndUpdate(
      req.params.id,
    ...
    console.error('Error updating appreciation file:', error);
...
const deleteFile = async (req, res) => {
  try {
    ...
    console.error('Error deleting appreciation file:', error);
...
const toggleLike = async (req, res) => {
  try {
    ...
    const populatedFile = await AppreciationFile.findById(req.params.id).populate('likes.user', 'fullName email');
...
const addComment = async (req, res) => {
  try {
    ...
    const comment = new AppreciationComment({
      content,
    ...
    if (parentComment) {
      await AppreciationComment.findByIdAndUpdate(parentComment, {
...
const getFileComments = async (req, res) => {
  try {
    const comments = await AppreciationComment.find({ 
      fileId: req.params.id, 
      isActive: true,
...
const updateComment = async (req, res) => {
  try {
    ...
    const comment = await AppreciationComment.findById(req.params.commentId);
