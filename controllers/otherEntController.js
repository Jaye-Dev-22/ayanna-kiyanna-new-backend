...
const OtherEntFolder = require('../models/OtherEntFolder');
const OtherEntFile = require('../models/OtherEntFile');
const OtherEntComment = require('../models/OtherEntComment');
...
const createFolder = async (req, res) => {
  try {
    ...
    const folder = new OtherEntFolder({ title, description, createdBy: req.user.id });
    await folder.save();
    ...
    console.error('Error creating otherEnt folder:', error);
...
const getAllFolders = async (req, res) => {
  try {
    const folders = await OtherEntFolder.find({ isActive: true })
      .populate('createdBy', 'fullName email')
      .sort({ createdAt: 1 });
    res.json({ success: true, data: folders });
  } catch (error) {
    console.error('Error fetching otherEnt folders:', error);
...
const getFolderById = async (req, res) => {
  try {
    const folder = await OtherEntFolder.findById(req.params.id)
      .populate('createdBy', 'fullName email');
    if (!folder) {
    ...
    console.error('Error fetching otherEnt folder:', error);
...
const updateFolder = async (req, res) => {
  try {
    ...
    const { title, description } = req.body;
    const folder = await OtherEntFolder.findByIdAndUpdate(
      req.params.id,
      { title, description },
      { new: true }
    ).populate('createdBy', 'fullName email');
    ...
    console.error('Error updating otherEnt folder:', error);
...
const deleteFolder = async (req, res) => {
  try {
    ...
    console.error('Error deleting otherEnt folder:', error);
...
const createFile = async (req, res) => {
  try {
    ...
    const folder = await OtherEntFolder.findById(folderId);
    if (!folder) {
      return res.status(404).json({ success: false, message: 'Folder not found' });
    }
    ...
    console.error('Error creating otherEnt file:', error);
...
const getFolderFiles = async (req, res) => {
  try {
    const files = await OtherEntFile.find({ folderId: req.params.folderId, isActive: true })
      .populate('createdBy', 'fullName email')
      .populate('folderId', 'title')
      .sort({ createdAt: 1 });
    res.json({ success: true, data: files });
  } catch (error) {
    console.error('Error fetching otherEnt files:', error);
...
const getFileById = async (req, res) => {
  try {
    const file = await OtherEntFile.findById(req.params.id)
      .populate('createdBy', 'fullName email')
      .populate('folderId', 'title')
    ...
    console.error('Error fetching otherEnt file:', error);
...
const updateFile = async (req, res) => {
  try {
    ...
    console.error('Error updating otherEnt file:', error);
...
const deleteFile = async (req, res) => {
  try {
    ...
    console.error('Error deleting otherEnt file:', error);
...
const toggleLike = async (req, res) => {
  try {
    ...
    if (existingLike) {
      ...
      updatedFile = await OtherEntFile.findByIdAndUpdate(
        req.params.id,
...
const addComment = async (req, res) => {
  try {
    ...
    const comment = new OtherEntComment({
      content,
    ...
    if (parentComment) {
      await OtherEntComment.findByIdAndUpdate(parentComment, {
...
const getFileComments = async (req, res) => {
  try {
    const comments = await OtherEntComment.find({ 
      fileId: req.params.id, 
      isActive: true,
...
const updateComment = async (req, res) => {
  try {
    ...
    const comment = await OtherEntComment.findById(req.params.commentId);
